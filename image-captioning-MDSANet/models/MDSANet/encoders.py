...
class MultiLevelEncoder(nn.Module):
    def __init__(self, N, padding_idx, d_model=512, d_k=64, d_v=64, h=8, d_ff=2048, dropout=.1, M=3, p=0.4,
        ...
        self.self_att = ScaledDotProductAttention(d_model=d_model, d_k=d_k, d_v=d_v, h=h, comment=None)

        self.mlp1 = nn.Linear(d_model * 3, d_model * 3)
        self.mlp2 = nn.Linear(d_model * 3, d_model)
    ...
    def forward(self, input, attention_weights=None):
        ...
        out2 = 0.2 * self.self_att(out2, out1, out1) + out2
        out3 = 0.2 * self.self_att(out3, out2, out2) + out3
