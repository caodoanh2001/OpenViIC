...
from models.MDSANet import Transformer, TransformerEncoder, TransformerDecoderLayer, ScaledDotProductAttention
import torch
    ...
    with tqdm(desc='Epoch %d - validation' % e, unit='it', total=len(dataloader)) as pbar:
        with torch.no_grad():
            for it, (detections, captions) in enumerate(dataloader):
                detections, image_id = detections['grids'], detections['image_id']
...
def evaluate_metrics(model, dataloader, text_field):
    ...
    with tqdm(desc='Epoch %d - evaluation' % e, unit='it', total=len(dataloader)) as pbar:
        for it, (images, caps_gt) in enumerate(iter(dataloader)):
            images, image_id = images['grids'], images['image_id']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--rl_at', type=int, default=18)
    parser.add_argument('--seed', type=int, default=201228)
    parser.add_argument('--warmup', type=int, default=10000)
    parser.add_argument('--resume_last', action='store_true')
    parser.add_argument('--resume_best', action='store_true')
    parser.add_argument('--features_path', type=str, default='../x152++_faster_rcnn')
    ...
    for e in range(start_epoch, start_epoch + 100):
        ...
        #Learning rate
        writer.add_scalar('data/learning_rate', optim.state_dict()['param_groups'][0]['lr'], e)
